...
WITH activite_externalisee_par_service AS (
    SELECT DISTINCT
        donnees ->> 'idService' as id_service,
        first_value(date) over par_service as date,
        first_value(donnees -> 'activitesExternalisees') over par_service as activitesExternalisees
    FROM journal_mss.evenements
    WHERE type = 'COMPLETUDE_SERVICE_MODIFIEE'
      AND donnees->>'activitesExternalisees' IS NOT NULL
...
FROM activite_externalisee_par_service
